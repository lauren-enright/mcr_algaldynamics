...
macroalgae %>% 
  ...
  rename(coral_cover = coral,
         sand_cover = sand) %>% 
  mutate(macroalgae_cover = rowSums(macroalgae[startAlg:endAlg]), # percent cover of algae
         macroalgae_cover_hard = case_when(
           sand_cover == 100 ~ NA, # since we can't divide by a zero
           TRUE ~ (macroalgae_cover/(100-sand_cover))*100 
           ),
         # also for corals
         coral_cover_hard = case_when(
           sand_cover == 100 ~ NA, # since we can't divide by a zero
           TRUE ~ (coral_cover/(100-sand_cover))*100 
           )
  ...
  select(year, location, site, habitat, island_side, site_habitat, 
         ...
         macroalgae_cover_hard, sand_cover, coral_cover, coral_cover_hard)  -> sppMeta
```
...
sppMeta %>% 
  ...
  mutate(macroalgae_richness = specnumber(sppMat),
         macroalgae_shannon = diversity(sppMat, "shannon")) -> macroAlgDiv
```
...
```{r save}
 write_csv(quadData, here("data/macroalgae_diversity_quadrat.csv"))
```
...
```{r save dataframe}
 write_csv(siteData, here("data/macroalgae_diversity_site.csv"))
```
...
```{r save}
write_csv(alltimeCV, here(here("data/macroalgae_stability_metrics_all_time.csv")))
```
