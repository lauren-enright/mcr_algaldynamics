...
t.s4.a <- as.tibble(emtrends(rich_stab_mod, pairwise ~ habitat, var = "richness_mean")$emtrends) %>% 
  ...
  mutate( #need to change letters by hand if post hoc contrasts change
    Letter = case_when(
      ...
      habitat == "Forereef 17m"  ~ "a"))

# get estimates for fucntional richness
t.s4.b <- as.tibble(emtrends(rich_stab_mod_fg, pairwise ~ habitat, var = "functional_richness_mean")$emtrends) %>% 
...
t.s4.c <- as.tibble(emtrends(synch_stab_mod, pairwise ~ habitat, var = "synchrony")$emtrends) %>% 
