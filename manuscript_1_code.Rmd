...
# ggsave(filename = "output/figure_2v2.png", figure_2, height = 10, width = 22)
```
...
functional_richness_quad_cover <- merge(alpha_diversity_quad_macro_with_cover, fg_summary_wide,
                                        by = c("location", "year", "site", "habitat"))

cover_mod_fg <- glmmTMB(cover_trans ~ fg_richness_quad*habitat + (1|site/location) + (1|year), data = functional_richness_quad_cover)


summary(cover_mod_fg)
car::Anova(cover_mod_fg)
#                              Chisq Df Pr(>Chisq)    
# fg_richness_quad         10323.133  1  < 2.2e-16 ***
# habitat                     87.166  3  < 2.2e-16 ***
# fg_richness_quad:habitat    77.758  3  < 2.2e-16 ***
hist(residuals(cover_mod_fg)) # a little positive skew, not the worst

r.squaredGLMM(cover_mod_fg) # 0.39, 0.51
emtrends(cover_mod_fg, pairwise ~ habitat, var = "fg_richness_quad") 
#all habitats are different from forereef 17
...
functional_richness_site_cover<- merge(alpha_diversity_site_macro_with_cover, fg_summary_site_wide,
...
diversity_ranges_site <- functional_richness_site_cover %>%
  group_by(habitat) %>%
...
(figure_3e <- 
  ...
  scale_colour_manual(values = habitat_colours) +
  scale_fill_manual(values = habitat_colours) +
  labs(y = "", x = "Functional Richness", title = "3e.") +
  theme_bw() + 
  theme(axis.title.x = element_text(size=24), 
        axis.title.y = element_text(size=24), 
        axis.text.x = element_text(size=22), 
...
# ggsave(filename = "output/figure_3.png", figure_3, height = 8, width = 16)
```
