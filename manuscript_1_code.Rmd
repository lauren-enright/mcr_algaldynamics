...
macro_long_data$area <- macro_long_data$prop_cover*0.25
# write_csv(macro_long_data, here("Data", "macro_long_data.csv"))
```


```{r site macroalgal data}
# NOTE - FRI JAN 24 - updated mean() in summarize across to sum() to reflect new method of calculating cover as area
site_macro <- macroalgae %>%
  dplyr::group_by(year, site, habitat,site_habitat) %>%
  dplyr::summarize(across(acanthophora_spicifera:valonia_ventricosa, \(x) sum(x, na.rm = TRUE))) 
...
alpha_diversity_quad_macro$area <- alpha_diversity_quad_macro$macroalgal_prop_cover*0.25
```
...
alpha_diversity_site_macro$area <- alpha_diversity_site_macro$prop_cover*0.25
```
...
# add small amount to avoid 0s in data for gamma modeling
alpha_diversity_quad_macro$area_trans <- alpha_diversity_quad_macro$area + 10E-10
...
mean_adiv_quad <- alpha_diversity_quad_macro %>% 
  ...
  dplyr::summarise(
    ...
    area_mean = mean(area_trans, na.rm = T))
...
# area model - play around with area vs. area_trans. If you use area_trans, family = Gamma("log") and remove ziformula argument
area_mod <- glmmTMB(area ~ richness*habitat + (1|site/location) + (1|year), family = ziGamma("log"), ziformula = ~1, data = alpha_diversity_quad_macro)
summary(area_mod)
Anova(area_mod)
hist(residuals(area_mod))
plot(simulateResiduals(area_mod))
plot(residuals(area_mod) ~ fitted(area_mod)) # heteroskedastic - because of the few 100% cover plots?
r.squaredGLMM(area_mod) # 0.631, 0.650
performance::r2(area_mod) # 0.635, 0.654
emtrends(area_mod, pairwise ~ habitat, var = "richness")
...
(figure_3a <- 
  ...
  geom_point(data = alpha_diversity_quad_macro,
             aes(x = richness, y = cover_trans, colour = habitat), size = 1, alpha = 0.5) + 
...
(figure_3b <- 
  ggplot() +
  geom_point(data = alpha_diversity_quad_macro,
             aes(x = functional_richness, y = cover_trans, colour = habitat), size = 1, alpha = 0.5) + 
...
### full vs mean plot comparison plot
```{r ggpubr meanfull, fig.height = 8, fig.width=9}

(mean_full_fxn_plot <- ggarrange(figure_3a, figure_3a_2, figure_3b, figure_3b_2, 
                      ncol = 2, nrow = 2, 
                      common.legend = TRUE,
                      legend = "bottom", 
                      font.label = list(size = 26, color = "black", face = "plain")))
...
# ggsave(filename = "output/figure_3v2.png", figure_3, height = 8, width = 16)
```
```{r figure 3 with mean, fig.height = 8, fig.width=9}
(figure_3_2 <- ggarrange(figure_3a_2, figure_3b_2, figure_3d, figure_3e, 
                      ncol = 2, nrow = 2, 
                      common.legend = TRUE,
                      labels = c("a.", "b.", "c.", "d."),
                      legend = "bottom", 
...
# ggsave(filename = "output/figure_3v3.png", figure_3_2, height = 8, width = 16)
