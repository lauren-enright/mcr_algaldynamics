...
## figure 4c model - richness synchrony
...
plot(residuals(rich_synch_mod) ~ fitted(rich_synch_mod)) # same issue as always
r.squaredGLMM(rich_synch_mod) # 0.701, 0.775
emtrends(rich_synch_mod, pairwise ~ habitat, var = "richness_mean") # forereefs aren't different
```
...
(figure_4c <- 
  ...
  geom_line(data = filtered_rich_synch_effects, aes(x = richness_mean, y = inv_logit(yvar), colour = habitat)) +
