...
columns_to_keep <- apply(species_matrix, 2, function(x) any(x >= 10))
...
# 
stratum_beta_disper <- vegan::betadisper(bray_matrix, wide_transect_level_subset$Stratum)
# plot(site_habitat_beta_disper, sub = "") 
...
stratum_distances$Stratum <- wide_transect_level_subset$Stratum
stratum_distances$Transect <- wide_transect_level_subset$Transect
stratum_distances$Year <- wide_transect_level_subset$Year
stratum_distances$Site <- wide_transect_level_subset$Site
stratum_distances$Habitat <- wide_transect_level_subset$Habitat
stratum_distances$Site_habitat <- wide_transect_level_subset$Site_habitat
...
diversity_stability <- alpha_diversity %>%
  dplyr::group_by(Site, Habitat, Site_habitat, Transect) %>%
  summarise(Richness_mean = mean(Richness), 
            Richness_se = sd(Richness)/sqrt(n()),
...
plot(allEffects(diversity_mod_1)) # richness reduces cv (i.e. enhances stability!)
...
diversity_mod_7 <- glmmTMB(Cover_cv ~ Shannon_mean*Site + (1|Site_habitat), family = Gamma(link = "log"), data = diversity_stability)
summary(diversity_mod_7)
Anova(diversity_mod_7)

diversity_mod_universal <- glmmTMB(Cover_cv ~ Shannon_mean*Site*Habitat + (1|Site_habitat), family = Gamma(link = "log"), data = diversity_stability, na.action = na.pass)
diversity_dredge <- dredge(diversity_mod_universal)
model.sel(diversity_dredge)

# best model based on dredge:
diversity_best <-  glmmTMB(Cover_cv ~ Shannon_mean*Site + Site*Habitat + (1|Site_habitat), family = Gamma(link = "log"), data = diversity_stability)
summary(diversity_best) # very low variance in random effect, which is great
Anova(diversity_best) # Interactions all significant
plot(dp <- simulateResiduals(diversity_best)) # some outliers, but otherwise good model fit. I am fine with this. 
r.squaredGLMM(diversity_best) # 0.77, 0.77
...
(shannon_cv_plot_2 = 
  ggplot(data = diversity_stability) +
  geom_point(aes(x = Shannon_mean, y = Cover_cv, colour = Site, shape = Habitat), size = 8, stroke = 2) + 
   scale_colour_manual(values = site_colours) +
   scale_shape_manual(values = habitat_shapes) +
  facet_wrap(~Site) +
  theme_bw() + 
...
```{r turnover, fig.height = 30, fig.width=25}
library(codyn)
...
turnover_means <-
  total_turnover %>%
  dplyr::group_by(Year, Site, Habitat, Site_habitat) %>%
  summarise(Turnover_mean = mean(total), 
...
appearance_means <-
  appearance %>%
  dplyr::group_by(Year, Site, Habitat, Site_habitat) %>%
  summarise(Appearance_mean = mean(appearance), 
...
disappearance_means <-
  disappearance %>%
  dplyr::group_by(Year, Site, Habitat, Site_habitat) %>%
  summarise(Disappearance_mean = mean(disappearance), 
...
rank_shift_means <-
  rank_shift %>%
  dplyr::group_by(Year, Site, Habitat, Site_habitat) %>%
  summarise(Rank_shift_mean = mean(MRS), 
