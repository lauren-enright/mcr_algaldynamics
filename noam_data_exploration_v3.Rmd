...
richness_mod_2 <- glmmTMB(
  richness ~ year_factor*habitat*island_side + (1 | island_side/transect/location) + ar1(year_factor + 0 | location),
  family = poisson,
  data = alpha_diversity_quad,
...
rownames(species_matrix) <- data$location
...
brayfull <- vegdist(species_matrix, "bray")
# View(brayfull)

# playing around with stuff here - ignore for now
x <- subset(data_subset, location == "lter_1_backreef_algae_transect_1_quad_1")
year_combinations <- expand.grid(Year1 = x$year, Year2 = x$year)
year_combinations <- year_combinations[year_combinations$Year1 != year_combinations$Year2, ] # filter out year 1 == year 2
year_combinations$Year_Difference <- abs(year_combinations$Year1 - year_combinations$Year2) # find difference in years
year_combinations$location <- rep("lter_1_backreef_algae_transect_1_quad_1", nrow(year_combinations))
year_combinations$bray <- NA
for(i in 1:nrow(year_combinations)){
  subset(x, year == year_combinations[i,1] | year_combinations[i,2])
}
