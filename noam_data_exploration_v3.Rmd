...
meta_cols <- c("year", "location", "site", "habitat", "site_habitat", "transect", "quadrat", "island_side", "island_side_habitat", "stratum")
...
for(i in 1:length(unique(alpha_diversity_quad$location)) ){ # for each  quadrat
  ...
  loop_mod <- gls(richness ~ year, correlation =corAR1(form=~year), 
...
```{r gls shannon, fig.height=10, fig.width=14}
alpha_diversity_quad$logShann <- log(alpha_diversity_quad$shannon + 1)
columns <- c("location", "Intercept", "Slope", "Phi") # names of columns for dataframe
gls_shann_coefficients <- # new dataframe name
  data.frame(matrix(nrow = length(unique(alpha_diversity_quad$location)), # 1200 quadrats = 1200 different time series
                    ncol = length(columns)) # specified to make sure number of columns doesn't exceed what was specified
             )
colnames(gls_shann_coefficients) <- columns # change column names to what was specified
gls_shann_coefficients$location <- levels(alpha_diversity_quad$location) # populate location column with each quad/time series
...
for(i in 1:length(unique(alpha_diversity_quad$location)) ){ # for each  quadrat
  ...
  # fit gls for current time series
  loop_mod <- gls(shannon ~ year, correlation =corAR1(form=~year), control = list(singular.ok = TRUE), data = newData)
  
  # populate appropriate row and column with the relevant coeffcients from gls
  gls_shann_coefficients[i, 2] <- unname(coef(loop_mod)[1])
  gls_shann_coefficients[i, 3] <- unname(coef(loop_mod)[2])
  gls_shann_coefficients[i, 4] <- loop_mod[["modelStruct"]][["corStruct"]]
  
}

hist(gls_shann_coefficients$Slope) # looks basically normal, bounded -Inf, Inf

hist(gls_shann_coefficients$Intercept) # same with intercept - although it probably shouldn't be
hist(gls_shann_coefficients$Phi)



gls_shann_coefficients_merged <- merge(gls_shann_coefficients, extra_meta, by = "location")             

slope_shann_mod <- glmmTMB(Slope ~ habitat*island_side + (1|island_side/site/transect), data = gls_shann_coefficients_merged)
summary(slope_shann_mod) # pretty low variance in random effects
plot(gls_res <- simulateResiduals(slope_shann_mod)) 

hist(residuals(slope_shann_mod))
plot(predict(slope_shann_mod) ~ residuals(slope_shann_mod))
Anova(slope_shann_mod)

r.squaredGLMM(slope_shann_mod) # 0.19, 0.22
plot(allEffects(slope_shann_mod))


slope_emm <- emmeans(slope_shann_mod, ~ habitat*island_side) # create emmeans object
slope_plot_info <- plot(slope_emm, plotit = FALSE)
(emmeans_slope_graph <- 
  ggplot(data =slope_plot_info) +
  geom_point(aes(y = the.emmean, x = habitat, colour = island_side), size = 8, position = position_dodge(0.3)) +
...
# Note: not sure I want to include this
emmeans(slope_shann_mod, pairwise ~ island_side | habitat)
# north and east coast not different from each other in backreef
# east and west not different from each other in fringing
# north and west not different from each other outer 10
# no sites different from each other outer 17
...
```{r glmm shannon, fig.height = 15, fig.width=25}
shannon_mod <- glmmTMB(
  log(shannon+1) ~ year_factor*habitat*island_side + (1 | site/transect/location) + ar1(year_factor + 0 | location),
  
  data = alpha_diversity_quad,
  control = glmmTMBControl(optCtrl = list(iter.max = 2000, eval.max = 2000)), na.action = na.pass
) # overspecified but going with it for now

summary(shannon_mod) # looks like strong autocorrelation, which makes sense to me
Anova(shannon_mod)

mod_emm <- emmeans(shannon_mod, ~ habitat * island_side * year_factor) # if we want to compare habitats year to year
shannon_plot_info <- plot(mod_emm, plotit = FALSE)
shannon_plot_info$year_numeric <- as.numeric(levels(shannon_plot_info$year_factor))
(emmeans_graph <- 
  ggplot(data =shannon_plot_info) +
  geom_point(aes(y = exp(the.emmean), x = year_numeric, colour = island_side), size = 8) +
  geom_errorbar(aes(ymin = exp(lower.CL), ymax = exp(upper.CL), x = year_numeric, colour = island_side), 
                width = 0.2, size = 1.5) +
  geom_segment(aes(y = 2.5, yend = 2.5, x = 2006, xend = 2010), linewidth = 4, alpha = 0.2, colour = "grey") +
  geom_vline(xintercept = 2010, linetype = 2, colour = "black", size = 2) +
  geom_vline(xintercept = 2019, linetype = 2, colour = "red", size = 2) +
  scale_y_log10() +
  facet_wrap(~habitat) +
  theme_classic() +
  labs(x = "Year", y = "Shannon-Weiner Index (H')\n") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
#        panel.background = element_blank(),
        legend.text = element_text(size=20),
...
```{r mean rank shift, fig.height = 24, fig.width=16, eval = FALSE}
# rank shift 
...
```{r rate of change, fig.height=15, fig.width=15, eval = FALSE}
# rate change
...
```{r rate change vis method 2, fig.height=15, fig.width=15, eval = FALSE}
...
## FOR FILTERING MODEL EFFECTS LATER
diversity_ranges <- diversity_stability_quadrat %>%
  group_by(island_side, habitat) %>%
  summarise(min_richness = min(richness_mean),
            max_richness = max(richness_mean),
            min_shannon = min(shannon_mean),
            max_shannon = max(shannon_mean),
...
emtrends(richness_cv_mod, pairwise ~ island_side | habitat, var = "richness_mean")
richness_cv_emm <- emmip(richness_cv_mod,
                        habitat ~ richness_mean | island_side,
                        at = list(richness_mean = seq(0,5,0.1)), plotit = F, CIs = T)

filtered_richness_effects <- richness_cv_emm %>%
  left_join(diversity_ranges, by = c("island_side", "habitat")) %>%
...
(richness_cv_plot = 
  ...
  geom_line(data = filtered_richness_effects, aes(x = richness_mean, y = exp(yvar), colour = island_side), size = 2) +
  geom_ribbon(data = filtered_richness_effects, aes(x = richness_mean, ymin = exp(LCL), ymax = exp(UCL), fill = island_side), alpha = 0.3) +
#   scale_colour_manual(values = site_colours) +
  scale_shape_manual(values = habitat_shapes) +
  facet_wrap(~habitat) +
  scale_y_log10() +
...
shannon_cv_mod <- glmmTMB(cover_cv ~ shannon_mean*habitat*island_side + (1|site/transect), family = Gamma(link = "log"),
                          data = diversity_stability_quadrat, na.action = na.pass)
# model.sel(dredge(shannon_cv_mod)) # top model is as specified, second best (delta = 10) doesn't include shann:hab:coast
r.squaredGLMM(shannon_cv_mod) # 0.55, 0.59
summary(shannon_cv_mod)
emtrends(shannon_cv_mod, pairwise ~ island_side | habitat, var = "shannon_mean")
shannon_cv_emm <- emmip(shannon_cv_mod,
                        habitat ~ shannon_mean | island_side,
                        at = list(shannon_mean = seq(0,1.25,0.05)), plotit = F, CIs = T)

filtered_shannon_effects <- shannon_cv_emm %>%
  left_join(diversity_ranges, by = c("island_side", "habitat")) %>%
  filter(shannon_mean >= min_shannon & shannon_mean <= max_shannon) 

(shannon_cv_plot = 
  ggplot() +
  geom_point(data = diversity_stability_quadrat,
             aes(x = shannon_mean, y = cover_cv, colour = island_side, shape = habitat), size = 2, alpha = 0.5) + 
  geom_line(data = filtered_shannon_effects, aes(x = shannon_mean, y = exp(yvar), colour = island_side), size = 2) +
  geom_ribbon(data = filtered_shannon_effects, aes(x = shannon_mean, ymin = exp(LCL), ymax = exp(UCL), fill = island_side), alpha = 0.3) +
#   scale_colour_manual(values = site_colours) +
  scale_shape_manual(values = habitat_shapes) +
  facet_wrap(~habitat) +
  scale_y_log10() +
...
synchrony_cv_effects <- emmeans(synchrony_cv_mod, pairwise ~ island_side | habitat, var = "loreau_synchrony")
...
filtered_synchrony_effects <- synchrony_cv_emm %>%
  left_join(diversity_ranges, by = c("island_side", "habitat")) %>%
...
(loreau_stability_plot_quad = 
  ...
  geom_line(data = filtered_synchrony_effects, aes(x = loreau_synchrony, y = exp(yvar), colour = island_side), size = 1) +
  geom_ribbon(data = filtered_synchrony_effects, aes(x = loreau_synchrony, ymin = exp(LCL), ymax = exp(UCL), fill = island_side),
             alpha = 0.3) +
   scale_shape_manual(values = habitat_shapes) +
  facet_wrap(~habitat) +
   scale_y_log10() +
...
synchrony_shann_mod <- glmmTMB(loreau_synchrony_trans ~ shannon_mean*habitat*island_side + (1|island_side/site/transect), 
                             family = beta_family(),
                          data = diversity_stability_quadrat, na.action = na.pass)
# model.sel(dredge(synchrony_shann_mod)) # top model is as specified
summary(synchrony_shann_mod)
synchrony_shann_effects <- emtrends(synchrony_shann_mod, pairwise ~ island_side | habitat, var = "shannon_mean")
synchrony_shann_emm <- emmip(synchrony_shann_mod,
                        habitat ~ shannon_mean | island_side,
                        at = list(shannon_mean = seq(0,1.25,0.05)), plotit = F, CIs = T)


filtered_synchrony_div_effects <- synchrony_shann_emm %>%
  left_join(diversity_ranges, by = c("island_side", "habitat")) %>%
  filter(shannon_mean >= min_shannon & shannon_mean <= max_shannon) 
...
(loreau_diversity_plot_quad <-
  ggplot() +
  geom_point(data = diversity_stability_quadrat,
             aes(x = shannon_mean, y = loreau_synchrony, colour = island_side, shape = habitat), size = 2, alpha = 0.5) + 
  geom_line(data = filtered_synchrony_div_effects, aes(x = shannon_mean, y = inv_logit(yvar), colour = island_side), size = 1) +
  geom_ribbon(data = filtered_synchrony_div_effects, aes(x = shannon_mean, ymin = inv_logit(LCL), ymax = inv_logit(UCL),
                                                         fill = island_side),
             alpha = 0.3) +
   scale_shape_manual(values = habitat_shapes) +
  facet_wrap(~habitat) +
...
synchrony_rich_mod <- glmmTMB(loreau_synchrony_trans ~ richness_mean*habitat*island_side + (1|island_side/site/transect), 
                             family = beta_family(),
                          data = diversity_stability_quadrat, na.action = na.pass)
# model.sel(dredge(synchrony_rich_mod)) # top model is as specified
summary(synchrony_rich_mod)
synchrony_rich_effects <- emtrends(synchrony_rich_mod, pairwise ~ island_side | habitat, var = "richness_mean")
synchrony_rich_emm <- emmip(synchrony_rich_mod,
                        habitat ~ richness_mean | island_side,
                        at = list(richness_mean = seq(0,5,0.1)), plotit = F, CIs = T)


filtered_synchrony_rich_effects <- synchrony_rich_emm %>%
  left_join(diversity_ranges, by = c("island_side", "habitat")) %>%
  filter(richness_mean >= min_richness & richness_mean <= max_richness) 

(loreau_richness_plot_quad <-
  ggplot() +
  geom_point(data = diversity_stability_quadrat,
             aes(x = richness_mean, y = loreau_synchrony, colour = island_side, shape = habitat), size = 2, alpha = 0.5) + 
  geom_line(data = filtered_synchrony_rich_effects, aes(x = richness_mean, y = inv_logit(yvar), colour = island_side), size = 1) +
  geom_ribbon(data = filtered_synchrony_rich_effects, aes(x = richness_mean, ymin = inv_logit(LCL), ymax = inv_logit(UCL),
                                                         fill = island_side),
             alpha = 0.3) +
   scale_shape_manual(values = habitat_shapes) +
  facet_wrap(~habitat) +
