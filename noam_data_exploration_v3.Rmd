...
```{r glmm approach, fig.height = 20, fig.width=15}
...
(emmeans_graph <- 
  ...
  geom_point(aes(x = exp(the.emmean), y = year_factor, colour = island_side), size = 8) +
  geom_errorbar(aes(xmin = exp(asymp.LCL), xmax = exp(asymp.UCL), y = year_factor, colour = island_side), width = 0.2, size = 1.5) +
  facet_wrap(~habitat) +
  theme_classic() +
  labs(y = "Year\n", x = "Estimated marginal mean richness") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
#        panel.background = element_blank(),
        legend.text = element_text(size=20),
...
total_turnover_mod1 <- glmmTMB(Turnover_trans ~ year_factor*habitat*island_side +
                                 (1 | island_side/site/transect) + ar1(year_factor + 0 | location),
