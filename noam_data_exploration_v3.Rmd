...
x <- subset(wide_transect_level, Stratum == "LTER 1 - Backreef - 1")

y <- x[ , -which(names(x) %in% meta_cols)]
y <- y[, which(colSums(y) != 0)]
z <- pe_avg_cv(y, ci = TRUE, detrending = "loess_detrended")
...
```{r diversity stability quadrat level}
