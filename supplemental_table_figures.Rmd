...
```{r}
supp_tables <- read_csv(here("data/supplemental_tables_manual_letters.csv"))
```
...
Plot-level stability

```{r}
supp_tables %>% 
  filter(Table == "S4") %>% 
  filter(Predictor == "Plot-level stability") %>% 
  # order the habitats by distance from shore
  mutate(Habitat = factor(Habitat, levels = c("Forereef 17m", "Forereef 10m",  "Backreef", "Fringing"))) %>% 
  ggplot(aes(x = Mean, y = Habitat, color = Habitat)) +
  geom_point(size = 6) +
  ...
  xlab("Coefficient") +
  ylab("") +
  ggtitle("(e) Plot-level stability") +
  model_themes + 
  # add letters denoting significance
  geom_text(aes(x = Upper_CI + 0.1 , y = Habitat, 
                label = Letter), colour = "black", size = 10) -> s4.plt.stab
```
...
supp_tables %>% 
  ...
  geom_text(aes(x = Upper_CI + 0.4 , y = Habitat, 
                label = Letter), colour = "black", size = 10) +
  theme(legend.position = "none") -> s4.spat.synch
```
...
(s4.taxon + s4.functional + s4.synch) /
  (s4.plt.synch + s4.plt.stab + s4.spat.synch)
